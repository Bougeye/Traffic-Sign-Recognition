...
class Training_Loop:
    ...
    def train(self, dataset, out_folder=""):
        ...
        train_idx, val_idx = train_test_split(list(range(len(dataset))), test_size=0.2, random_seed=self.random_seed, stratify=labels)
    ...
    def generate_metrics(self,preds,targets,epoch):
        ...
        for i in range(len(label_cm)):
            label_acc[str(i)] = [(label_cm[i][0][0]+label_cm[i][0][1])/(label_cm[i][0][0]+label_cm[i][0][1]+label_cm[i][1][0]+label_cm[i][1][1])]
