...
class Training_loop:
    ...
    def _train_one(self, train_ds, val_ds, fold_id):
        ...
        train_loader = DataLoader(train_ds, shuffle=False, num_workers=4, persistent_workers=True,
                                  pin_memory = (device.type=="cuda"), batch_size=self.tr_config["bsize"])
                val_loader = DataLoader(val_ds, shuffle=False, num_workers=4, persistent_workers=True,
                                pin_memory=(device.type=="cuda"), batch_size=self.tr_config["bsize"])
        start = time.time()
