...
class evaluate:
    ...
    def evaluate_on(self, target="training", datapath=None, random_seed=42):
        ...
        concept_df = reports.metrics_report(concept_report, concept_names, f"reports/Eval-{self.ts}/concept_report.pdf")
        label_df = reports.metrics_report(label_report, label_names, f"reports/Eval-{self.ts}/label_report.pdf")
        reports.label_cm_report(self.pth_cfg["data"]["training"],label_cm, label_names, f"reports/Eval-{self.ts}/label_cm.pdf")
        plots.level_metrics(concept_df,f"reports/Eval-{self.ts}/concept_level_metrics")
        plots.level_metrics(label_df,f"reports/Eval-{self.ts}/label_level_metrics")
